...
- **Terminal** integrates the read-only `@swaputer-labs/cli` verifier with `inspect`, `decode-receipt`, `help`, `version` and `clear` commands.
- **Explore** opens the protocol explorer in a separate browser tab.

The browser-safe CLI and receipt codec releases are vendored under `vendor/`, so this frontend remains independently installable and Docker-buildable without relying on the surrounding monorepo checkout.
...
```sh
docker build --build-arg VITE_PROTOCOL_EXPLORER_URL=https://YOUR_EXPLORER_HOST -t swaputer-computer .
