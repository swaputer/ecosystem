  ...
  <main class="page market-detail-page market-gallery-page">
      ...
      <section class="market-gallery-content" :aria-busy="activeTable.loading">
        ...
        <div class="market-gallery-scroll">
          ...
          <div v-else-if="view === 'activity'" class="protocol-table-wrap market-trades-table"><table>
            ...
            <tbody><tr v-for="trade in tradeTable.items" :key="`${trade.transactionHash}-${trade.orderId}`"><td>{{ formatAge(trade.blockTime) }}</td><td>{{ formatUnitsExact(trade.amount, decimals, 5) }}</td><td>{{ priceETH(trade.priceWei) }}</td><td><a :href="explorerURL(`/address/${trade.buyer}`)" target="_blank" rel="noopener noreferrer">{{ shortHex(trade.buyer, 8, 6) }}</a></td><td><a :href="explorerURL(`/address/${trade.seller}`)" target="_blank" rel="noopener noreferrer">{{ shortHex(trade.seller, 8, 6) }}</a></td><td><a :href="explorerURL(`/tx/${trade.transactionHash}`)" target="_blank" rel="noopener noreferrer">{{ shortHex(trade.transactionHash, 9, 7) }}</a></td></tr></tbody>
          </table></div>
    ...
    <MarketDialog v-if="selectedOrder" :title="confirmationTitle" :busy="actionBusy" @close="selectedOrder = null">
      ...
      <dl class="market-confirm-details">
        ...
        <div><dt>Maker</dt><dd><a :href="explorerURL(`/address/${selectedOrder.maker}`)" target="_blank" rel="noopener noreferrer">{{ shortHex(selectedOrder.maker, 8, 6) }}</a></dd></div>
