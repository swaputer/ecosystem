...
const nativeBalance = ref<bigint | null>(null);
...
const balance = computed(() => {
  ...
  return nativeBalance.value === null ? "—" : nativeAmount(nativeBalance.value, 6);
});
...
async function refresh() {
  ...
  loading.value = true;
  try {
    const account = wallet.address.value;
    const [bridge, fees, ethBalance] = await Promise.all([
      readBridgeSnapshot(wallet.address.value),
      readProtocolFeeConfig().catch(() => null),
      account ? readProvider.getBalance(account).catch(() => null) : null
    ]);
    if (account !== wallet.address.value) return;
    snapshot.value = bridge;
    feeConfig.value = fees;
    nativeBalance.value = ethBalance;
  } catch (cause) {
  ...
  <main class="page page--bridge">
    ...
    <section class="bridge-layout">
        ...
        <div class="bridge-fields">
          <div class="amount-block">
            ...
            <div class="amount-row">
              ...
              <div class="asset-select asset-select--static"><span class="asset-glyph"><AssetGlyph :asset="inputAsset" /></span>{{ inputAsset }}</div>
            </div>
          ...
          <div class="amount-block amount-block--output">
            ...
            <div class="amount-row">
              ...
              <div class="asset-select asset-select--static"><span class="asset-glyph"><AssetGlyph :asset="outputAsset" /></span>{{ outputAsset }}</div>
            </div>
          </div>
        </div>
      ...
      <aside class="atomic-path">
        <h2>Atomic path</h2>
        <ol>
          <li><span>01</span><div><strong>{{ inputAsset }}</strong><p>{{ direction === 'deposit' ? 'ETH enters the Swaputer vault.' : 'Your sETH is returned to the vault.' }}</p></div></li>
          <li><span>02</span><div><strong>Vault</strong><p>{{ direction === 'deposit' ? 'The vault records the deposit and mints sETH.' : 'The vault burns sETH and releases its ETH backing.' }}</p></div></li>
          <li><span>03</span><div><strong>{{ outputAsset }}</strong><p>{{ direction === 'deposit' ? 'sETH is issued to your address.' : 'ETH is returned to your wallet.' }}</p></div></li>
        </ol>
